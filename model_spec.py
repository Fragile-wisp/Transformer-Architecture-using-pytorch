...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model:int, seq_len: int, dropout: float) -> None:
        ...
        #Creating mats of (seq_len, d_model) and vec of (seq_len, 1)
        pe = torch.zeros(seq_len, d_model)

        #position and div terms
        position = torch.arange(0, seq_len, dtype=torch.float).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, d_model, 2).float() * (-math.log(10000.0)/d_model))

        #Apply sin for even and cos for odd
...
def build_transformer(src_vocab_size: int, tgt_vocab_size: int, src_seq_len: int, tgt_seq_len: int, d_model: int, N: int, h: int, dropout: float = 0.1, d_ff: int = 2048) -> Transformer:
    #Embedding layers
